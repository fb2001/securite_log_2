...
function extractSmaliClassName(smaliContent) {
    const match = smaliContent.match(/^\s*\.class\s+.*\s+(L[^;]+;)\s*$/m);
    if (!match || !match[1]) {
        throw new Error("Impossible de trouver la ligne .class ... L...; dans le détecteur");
    }
    return match[1];
}
export function copyDetectorSmali(decodedDir, detectorFilePath) {
