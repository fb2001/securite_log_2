...
function listSmaliRoots(decodedDir) {
    return fs
...
function walkSmaliFiles(dir, out = []) {
    for (const ent of fs.readdirSync(dir, { withFileTypes: true })) {
        const p = path.join(dir, ent.name);
        if (ent.isDirectory())
            walkSmaliFiles(p, out);
        else if (ent.isFile() && ent.name.endsWith(".smali"))
            out.push(p);
    }
    return out;
}
 ...
 */
function extractDeclaredClass(tree) {
...
export function findSmaliFileForClass(decodedDir, wantedClass) {
    ...
    for (const root of roots) {
        ...
        for (const file of files) {
            ...
            if (!tree)
                continue; // fichier non parsable -> on skip
